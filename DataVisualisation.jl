...
begin
	using PlutoUI, PlutoTeachingTools, DataFrames, CSV, WGLMakie, Bonito, Observables, JLD2, FileIO, Colors, Electron, CairoMakie

	md"""
	"""
end |> WideCell

# ╔═╡ 797d359a-d4db-4fca-b0e8-a8577e35a24e
using Main.var"workspace#8".DashboardProject

# ╔═╡ 224aab58-eaaa-4b0f-b2b8-a25c1b59066d
include("DashboardProject/DashboardProject.jl")
...
# ╔═╡ d2b484bd-2283-450f-a72b-8f2af79f9633
begin 
	global path_to_csv = normpath(joinpath(homedir(),"Data/PQ_FFT/DF.csv"))
	ext = ".jld"
end

# ╔═╡ 1e53adfd-5822-4000-a4a6-29b23afed9e5
# include("VisualisationDashboard/VisualisationDashboard.jl") |> WideCell
...
# ╔═╡ aea7a08b-fb02-4d44-b081-8da55bb30065
html"""
<style>
    /* Target the current running cell and blow open its max-width limits */
    pluto-cell.focused, pluto-cell {
        max-width: 95vw !important; /* 95% of the total viewport width */
    }
    
    /* Ensure the output container scales to match the new cell boundaries */
    pluto-output {
...
begin
# 1. Define your custom style rule
style_override = html"""
<style>
    main {
        margin: 0 auto;
        max-width: 95% !important;     /* Opens up the width to 95% of your monitor */
        padding-left: 2% !important;    /* Drastically reduces huge side gutters */
        padding-right: 2% !important;   /* Drastically reduces huge side gutters */
    }

    /* Target Pluto's internal cell outputs to give your Makie app more height */
    pluto-output, .bonito-app {
        min-height: 85vh !important;    /* Forces container to take 85% of viewport height */
...
# 3. Combine them using Bonito's native DOM element instead of an HTML string wrapper
Bonito.DOM.div(
    style_override,
    Bonito.DOM.div(app, class="dashboard-fullscreen-container")
)
end

# ╔═╡ abea53fb-8aab-4426-8d00-f43338bc4dc6
# TwoColumn(md"", md""" $(df_in = @bind df_inputs build_df_inputs(name_list, tab_list))""")

# ╔═╡ e1057490-5252-4571-81b0-8dcd3e853116
# keys(df_inputs)
# values(df_inputs)[2] = true

# ╔═╡ 12675f84-a182-4a28-b799-82c4944d0ccd
# begin
# fig = Figure(size=(800,800))
# ax = Axis(fig[2,2])
# fig[2,1] = ui_grid = GridLayout(width=250, height=600)#, colsize=400)
...
# 			ui_sub_grid = GridLayout(ui_grid[h,1], height=30)
# 			# ui_sub_grid_lab = GridLayout(ui_grid[h,1])
# 			if isa(tab_list[i][1], AbstractString)
# 				Label(ui_sub_grid[h,1], text=name_list[i])
# 				push!(wid_df, Makie.Menu(ui_sub_grid[h,2], options=sort(tab_list[i])))
# 				push!(wid_df, Makie.Checkbox(ui_sub_grid[h,3], checked=false))
# 			else
# 				# Label(ui_sub_grid[h,1], text=name_list[i])
# 				push!(wid_df, Makie.SliderGrid(ui_sub_grid[h,1:2],(label=name_list[i], range=tab_list[i])))#, update_while_dragging=false))
# 				push!(wid_df, Makie.Checkbox(ui_sub_grid[h,3], checked=false));
# 			end
# 			h += 1
# 		end
# 		rowgap!(ui_grid,(600-50*N_line_wid_df)/N_line_wid_df)
# 	end
# end
# wid_df	
# end

# ╔═╡ 4265ad58-011e-4d9d-b609-db794580be58
# fig |> WideCell

# ╔═╡ 356321e0-96f9-4974-923c-c88e13d7a53c
# ╠═╡ disabled = true
# ╠═╡ skip_as_script = true
#=╠═╡
# # fig[1,1] = vgrid!([
# # 	# [Label(fig, changing_variable_list[i]),
# # 	wid_df[i]
# # 	for i in length(wid_df)]...;
# #     height = 25*18, width = 200
# # )
# # set_close_to!(wid_df[1].sliders[1], 2040)
# # wid_df[1].sliders[1]

# all_dir = JulUtils.get_all_dir_ext(dir; ext=ext)
# if Sys.iswindows()
#     all_dir .= replace.(all_dir, "\\" => "/")
# end

# pathdir = Observable{Any}(all_dir[1]*"/")


# Nfiles = maximum([length(readdir(d)) for d in all_dir])
# path_first_file = string(all_dir[1],"/",readdir(all_dir[1])[1])
# all_keys = collect(keys(FileIO.load(path_first_file)))
# Nkeys = length(all_keys)
# dim_of_data = zeros(Int,length(all_keys))
# for k_id in eachindex(all_keys)
#     dim_of_data[k_id] = ndims(FileIO.load(path_first_file, all_keys[k_id]))
# end

#    drop_heatmap = Bonito.Dropdown(sort(all_keys); index=1)
#     drop_colormap = Bonito.Dropdown(sort(collect(Makie.all_gradient_names)), index=1)

# Bonito.map(wid_all_df..., cb_all_df...) do args...
#         dist = zeros(Nrow)
#         for r=1:Nrow
#             for c=1:N_cb_df
#                 dist[r] += Int(df[r, changing_variable_list[c]] == args[c])*(1 + Int(args[c+N_cb_df])*100)
#             end
#         end
#         idx = findmax(dist)[2]
#         for c=1:N_cb_df
#             if args[c] != df[idx, changing_variable_list[c]]
#                 wid_all_df[c].value[] = df[idx, changing_variable_list[c]]
#             end
#         end
#         pathdir[] = string(dir, df[idx,:fn], "/Data/")
#     end
  ╠═╡ =#

# ╔═╡ ca5b3bd4-f21d-43f3-8f44-14d251312eb3
# fig |> WideCell
...
manifest_format = "2.0"
project_hash = "ffc2c0b15f9ddb8140007aeb71d16804bd0b5cb4"
